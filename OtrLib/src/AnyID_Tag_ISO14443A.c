...
u8 ISO14443A_CheckBcc(u8 *pUID)
{
    ...
    if(bcc == 0)
    {
        return ISO14443A_ERR_OK;
    }
    else
    {
        return ISO14443A_ERR_BCC;
    }
}
...
u8 ISO14443A_AnticollisionTag(u8 lv, u8 *pUID)
{
    u8 error = ISO14443A_ERR_OK;
#if defined(_ANYID_M325_)    
    g_sRc663Frame.frame[0] = 0x93;
    g_sRc663Frame.frame[1] = 0x20;
    g_sRc663Frame.txLen = 2;
    
    g_sRc663Frame.timeout = ISO14443A_NORMAL_TIME;
    g_sRc663Frame.cmd = RC663_CMD_TRANSCEIVE;

    RC663_Config(TRUE, FALSE, FALSE, 0, 0);
    error = RC663_Command();
    if(error == ISO14443A_ERR_OK)
    {
        error = ISO14443A_CheckBcc(g_sRc663Frame.frame);
    }
#elif defined(_ANYID_R321_)    
...
u8 ISO14443A_TransAPDU(u8 *pApdu, u8 len, u8 *pAsk, u8 *pAskLen)
{
    ...
    }while((error == ISO14443A_ERR_OK) && (pcb == (ISO14443A_PCB_SBLOCK | ISO14443A_SPCB_WTX)));
